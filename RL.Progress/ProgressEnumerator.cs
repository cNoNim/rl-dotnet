...
public struct ProgressGenerator<TG, T>(TG generator) : 
    ...
    public T this[int index]
    {
        get
        {
            ...
            if (index == Count - 1)
            {
                ...
                if (elapsed.HasValue) 
                    _sb.Append($" {elapsed:%m\\:ss\\.ffff} {Count / elapsed.Value.TotalSeconds:#.00}it/s");
            ...
            {
                var interval = Count / 100;
                if (index % interval != 0) 
                    return result;
                _sb ??= new StringBuilder();
                _sw ??= new Stopwatch();
                _sb.Clear();
                var progress = index / interval;
                _sb.Append('\b', _lastLength);
                _sb.Append($"{progress,3:#}%");
                ...
                {
                    var elapsed = _sw.Elapsed;
                    _sb.Append($" {elapsed:%m\\:ss\\.ffff} {interval / (elapsed - _lastElapsed.Value).TotalSeconds:#.00}it/s");
